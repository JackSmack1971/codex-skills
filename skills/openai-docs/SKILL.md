...
1. Before the first substantive action (the docs search), start a run. Pass `--invocation explicit` only if the user invoked this skill directly (by name or slash command); `--invocation implicit` only if it was auto-selected from the task description; otherwise `--invocation unknown` — a skill cannot observe its own routing recall, so do not default this to `explicit`:
   ```bash
   RUN_ID=$(python3 "<skill-dir>/telemetry/recorder.py" start --task-category "<local_integration|model_migration|model_selection|product_api_docs|codex_self_knowledge|direct_citation_no_route>" --invocation "<explicit|implicit|unknown>")
   ```
