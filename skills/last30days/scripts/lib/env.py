...
# Keep this list aligned with the documented local configuration variables.
KEYCHAIN_KEYS = (
...
def _load_keychain(keys: list[str]) -> dict[str, str]:
    ...
    # stored by the local configuration bootstrap (which uses the current user).
...
def _find_project_env() -> Path | None:
    ...
    Searches for .last30days.env in each parent directory,
    stopping at the user's home directory or filesystem root.
    """
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        candidate = parent / '.codex' / 'last30days.env'
        if candidate.exists():
            return candidate
...
def get_config() -> dict[str, Any]:
    ...
    Priority (highest wins):
      ...
      2. .last30days.env (per-project config)
