...
- [Docs-First Triggers](#docs-first-triggers)
- [What Counts As Docs](#what-counts-as-docs)
- [Required Workflow](#required-workflow)
- [Examples That Must Trigger Docs](#examples-that-must-trigger-docs)
- [When A Quick Local Read Is Enough](#when-a-quick-local-read-is-enough)
- [If Docs Are Unavailable](#if-docs-are-unavailable)
# Read The Damn Docs
...
1. Before Required Workflow step 1, start a run. Pass `--invocation explicit` only if the user invoked this skill directly (by name or slash command); `--invocation implicit` only if it was auto-selected from the task description; otherwise `--invocation unknown` — a skill cannot observe its own routing recall, so do not default this to `explicit`:
   ```bash
   RUN_ID=$(python3 "<skill-dir>/telemetry/recorder.py" start --task-category "<package_add_or_upgrade|provider_api_or_auth|error_or_deprecation|local_repo_contract|irreversible_choice|explicit_request>" --invocation "<explicit|implicit|unknown>")
   ```
