...
description: "Audit and professionalize GitHub repositories: stack and topology, stale worktrees, labels, .github governance and actions, documentation, settings, and tracked artifacts; create one idempotent issue per atomic step."
...
- **Trigger and exclusion:** Use for a repository-wide hygiene and governance audit; exclude a narrow code or PR review, routing to review-agent or pr-review.
...
1. Before Default-workflow step 1 (Preflight), start a run. Pass `--invocation explicit` only if the user invoked this skill directly (by name or slash command); `--invocation implicit` only if it was auto-selected from the task description; otherwise `--invocation unknown` — a skill cannot observe its own routing recall, so do not default this to `explicit`:
   ```bash
   RUN_ID=$(python3 "<skill-dir>/telemetry/recorder.py" start --task-category "<audit|audit_publish|maintenance|verify>" --invocation "<explicit|implicit|unknown>")
   ```
