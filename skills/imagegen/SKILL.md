---
name: "imagegen"
description: "Generate or edit raster images when bitmap visuals are the right deliverable."
...
1. Before Workflow step 1 (decide the top-level mode), start a run. Pass `--invocation explicit` only if the user invoked this skill directly (by name or slash command); `--invocation implicit` only if it was auto-selected from the task description; otherwise `--invocation unknown` — a skill cannot observe its own routing recall, so do not default this to `explicit`:
   ```bash
